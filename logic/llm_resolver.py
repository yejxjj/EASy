...
def _is_hallucinating(text):
    ...
    # 1. 핑계, 안내, 거절 등 LLM 특유의 답변 패턴 단어들
    ...
    # 2. 회사명/모델명 치고 너무 길면(40자 이상) 정상적인 단답형이 아님
    if len(text) > 40:
        return True
...
def resolve_real_company_name(brand_name, product_name=""):
    ...
    prompt = (
        ...
        f"규칙 4: 설명, 인사말, 문장 기호 없이 결과만 단답형으로 출력해.\n"
    ...
    if _is_hallucinating(result):
        print(f"   ⚠️ [LLM 헛소리 필터링 작동] 무의미한 응답 제거. 원본 브랜드명 '{brand_name}' 그대로 사용.")
        return brand_name

    if result:
        # 혹시 모를 줄바꿈 문자 제거 등 클렌징
